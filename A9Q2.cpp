...
int diestar(unordered_map<string,unordered_map<string,int>>&wordlist,string source, string target){
	// Boilerplate initalize
...
int main(){
	...
	while (cin >> source && cin >> target){
		if (wordlist.count(source) == 0 || wordlist.count(target) == 0){
			cout << "-1" << endl;
		}
		else{
			diestar(wordlist,source,target);
		}
	}
	return 0;
}
